...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/wyusgw/XrayR-release/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/wyusgw/XrayR-release/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/wyusgw/XrayR-release/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_menu() {
    echo -e "
  ${green}XrayR 后端管理脚本，${plain}${red}备份的${plain}
--- https://github.com/wyusgw/XrayR ---
  ${green}0.${plain} 修改配置
————————————————
